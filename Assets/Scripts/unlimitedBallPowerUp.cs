...
public class unlimitedBallPowerUp : MonoBehaviour {
	...
	void Update () {
		if ((startTime != -1) && (Time.time - startTime > 5)) {
			startTime = -1;
			colorChange = 0;
			numThrows = 0;
	...
	void endOfPowerUp() {
		currentPlayer.gameObject.GetComponent<PlayerController> ().throwing = false;
		currentPlayer.gameObject.GetComponent<PlayerController> ().justThrown = false;
